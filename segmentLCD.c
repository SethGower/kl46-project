...
/******************************************************************/
//						Function for Initialization of
// LCD
////
/******************************************************************/
...
void SegLCD_Init(void) { // Initializes all components of SLCD on the FRDM-KL46Z
    ...
    SIM->SCGC5 |= SIM_SCGC5_PORTB_MASK | SIM_SCGC5_PORTC_MASK |
                  ...
                  SIM_SCGC5_SLCD_MASK; // Enable Clock to ports B, C, D and E,
                                       // and SegLCD Peripheral
    LCD->GCR |=
        LCD_GCR_PADSAFE_MASK; // Set PADSAFE to disable LCD while configuring
    LCD->GCR &=
        ~LCD_GCR_LCDEN_MASK; // Clear LCDEN (LCD Enable) while configuring

    // Configure pins *From Reference manual, set pins to MUX 0 for normal LCD
    // display operation, only use MUX 7 if using LCD fault detection
    ...
    // Configure LCD_GCR - General Control Register, controls most options in
    // LCD Peripheral
    LCD->GCR = // LCD_GCR_RVEN_MASK |
               // //Clear LCD_GCR_RVEN, disable voltage regulator.
        LCD_GCR_RVTRIM(
            0x00) | // Set RVTRIM to 0, irrelevant as voltage regulator is
                    // disabled, but setting it to a known state.
        LCD_GCR_CPSEL_MASK | // Set LCD_GCR_CPSEL to use capacitor charge pump.
        LCD_GCR_LADJ(0x03) | // Set LCD_GCR_LADJ to 11, slow clock rate = lower
                             // power, but higher load capacitance on the LCD
                             // requires higher clock speed.
        // LCD_GCR_VSUPPLY |
        // //Clear LCD_GCR_VSUPPLY, drive VLL3 externally.
        LCD_GCR_PADSAFE_MASK | // Set LCD_GCR_PADSAFE, leave enabled during
                               // configuration process.
        // LCD_GCR_FDCIEN_MASK | 					//Clear
        // LCD_GCR_FDCIEN, No interrupt from fault dection.
        LCD_GCR_ALTDIV(
            0x00) | // Set LCD_GCR_ALTDIV to 11, divide alternate clock by 512.
                    // This is assuming an 8MHz External Crystal is used.
        // LCD_GCR_ALTSOURCE_MASK |				//Set
        // LCD_GCR_ALTSOURCE, Part of setting clock source to OSCERCLK, or
        // external oscillator.
        LCD_GCR_FFR_MASK | // Set LCD_GCR_FFR, allow an LCD Frame Frequency
                           // of 46.6Hz to 146.2Hz.  Disable to change range
                           // to 23.3Hz to 73.1Hz.
        // LCD_GCR_LCDDOZE_MASK |					//Clear
        // LCD_GCR_LCDDOZE, allows LCD peripheral to run even in doze mode.  Set
        // to disable LCD in doze mode.
        // LCD_GCR_LCDSTP_MASK |
        // //Clear LCD_GCR_LCDSTP, allows LCD peripheral to run even in stop
        // mode.  Set to disable LCD in stop mode. LCD_GCR_LCDEN_MASK |
        // //Clear LCD_GCR_LCDEN, Disables all front and backplane pins.  Leave
        // disabled during configuration process.
        LCD_GCR_SOURCE_MASK | // Set LCD_GCR_SOURCE, Part of setting clock
                              // source to OSCERCLK, or external oscillator.
        LCD_GCR_LCLK(
            0x04) | // Set LCD_GCR_LCLK to 111, LCD Clock prescaler where LCD
                    // controller frame frequency = LCD clock/((DUTY  |  1) x 8
                    // x (4 | LCLK[2:0]) x Y), where Y = 2, 2, 3, 3, 4, 5, 8, 16
                    // chosen by module duty cycle config
        LCD_GCR_DUTY(0x03); // Set LCD_GCR_DUTY to 011, Have 4 backplane pins,
                            // so need a 1/4 duty cycle.

    // Configure LCD_SEG_AR  - Auxiliary Register, controls blinking of LCD
    LCD->AR = // LCD_AR_BLINK_MASK |
              // //Clear LCD_SEG_AR_BLINK, Disable SLCD blinking.  Enable to
              // make LCD Blink. LCD_AR_ALT_MASK |
              // //Clear LCD_SEG_AR_ALT, if enabled LCD back plane sequencer
              // changes to an alternate display.  Only functional if DUTY[2:0]
              // is less than 100(binary).  This allows a blink screen that is
              // not blank. LCD_AR_BLANK_MASK |
              // //Clear LCD_SEG_AR_BLANK, asserting bit clears all segments in
              // LCD.
              // LCD_AR_BMODE_MASK |
              // //Clear LCD_SEG_AR_BMODE, if enabled displays alternate display
              // during blink period instead of blank.
        LCD_AR_BRATE(0x00); // Set LCD_SEG_AR_BRATE to 000.  Frequency of blink
                            // is determined by LCD clock/(2^(12 + BRATE))

    // Configure LCD_SEG_FDCR - Fault Detect Control Register, controls use of
    // Fault Detect features of SLCD.
    LCD->FDCR =
        0x00000000; // Clear all bits in FDCR.  As this will not be covering use
                    // of fault detect, this register is cleared.

    // Configure LCD_PENn - Pin Enable Register, controls which of the possible
    // LCD pins are used LCD->PEN[0] contains bits 0-31, while LCD->PEN[1]
    // contains bits 32-63.  For pins >= 32, set in LCD->PEN[1] as
    // LCD_PEN_PEN(Pin#-32)
    ...
    // Configure LCD_SEG_BPENn - Back Plane Enable Register, controls which pins
    // in LCD->PEN are Back Plane (commons) LCD->BPEN[0] contains bits 0-31,
    // while LCD->BPEN[1] contains bits 32-63.  For pins >= 32, set in
    // LCD->BPEN[1] as LCD_BPEN_BPEN(Pin#-32)
    ...
    // defined such that x = (z*4) and y = 3 | (z*4) Where x is the n index
    // value of WFn on the least significant byte (bits 7-0) and y is the n
    // index value of WFn on the most significant byte (bits 31-24)

    // Note that "Disabled" is used for pins that are not set as LCD pins, where
    // "Off" is used for pins that are set as LCD, but are just inactive.
...
/******************************************************************/
//						End Function for Initialization
// of LCD
////
/******************************************************************/
...
void SegLCD_Set(
    uint8_t value,
    uint8_t
        Digit) { // Sets a value from 0-F to a specified Digit, with 1 being the
                 // leftmost, 4 being the rightmost.  Will not display error is
                 // value is outside of 0-F, but display will not update
    if (Digit > 4) {
...
void SegLCD_DisplayDecimal(
    uint16_t Value) { // Displays a 4 Digit number in decimal
    if (Value > 9999) {
        SegLCD_DisplayError(
            0x10); // Display "Err" if value is greater than 4 digits
    } else {
...
void SegLCD_DisplayHex(
    uint16_t Value) { // Displays a 16 bit number in Hex Format
...
void SegLCD_DisplayTime(
    uint8_t Value1, uint8_t Value2) { // Displays 2 values separated by a colon
    if ((Value1 > 99) | (Value2 > 99)) {
        SegLCD_DisplayError(
            0x10); // Display "Err" if either value is greater than 2 digits
    } else {
...
void SegLCD_DisplayError(
    uint8_t ErrorNum) { // Displays Err# on screen, where # is a value 0-F.  If
                        // ErrorNum is outside of that range, just displays Err
    LCD->WF8B[LCD_FRONTPLANE0] =
        (LCD_SEG_D | LCD_SEG_E | LCD_SEG_F | LCD_SEG_G);
    ...
    if (ErrorNum < 0x10) {
        SegLCD_Set(ErrorNum, 4); // Display ErrorNum in digit 4 if within valid
                                 // range.  If not, leave blank.
    } else {
...
// SegLCD_DP1_On() defined as macro in Seg_LCD.h, Turns on leftmost decimal
// without disturbing rest of display SegLCD_DP1_Off() defined as macro in
// Seg_LCD.h, Turns off leftmost decimal without disturbing rest of display
// SegLCD_DP2_On() defined as macro in Seg_LCD.h, Turns on center decimal
// without disturbing rest of display SegLCD_DP2_Off() defined as macro in
// Seg_LCD.h, Turns off center decimal without disturbing rest of display
// SegLCD_DP3_On() defined as macro in Seg_LCD.h, Turns on rightmost decimal
// without disturbing rest of display SegLCD_DP3_Off() defined as macro in
// Seg_LCD.h, Turns off rightmost decimal without disturbing rest of display
// SegLCD_Col_On() defined as macro in Seg_LCD.h, Turns on colon without
// disturbing rest of display SegLCD_Col_Off() defined as macro in Seg_LCD.h,
// Turns off colon without disturbing rest of display

/******************************************************************/
//						End Functions for Manipulation
// of LCD
////
/******************************************************************/
